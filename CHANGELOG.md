...
- The new API, heavily inspired from tools like FastAPI or django-ninja, encapsulates the procedures registration in a
...
- `RpcNamespace` was added to provide a better organization of procedures. Each one can be registered in a
...
- Error handling has been improved, allowing execution of a callback function when an exception is caught and before an
...
- Builtin `system.methodSignature` behavior has been updated to respect standard. It now returns a list of
...
- All tests have been rewritten to have a strong separation between unit and functional tests. Test classes were
...
- Removed useless settings variable introduced in the v0.7.0 release. Logging capabilities are now enabled by simply
...
- Python 2 strings standardization. Allows configuring an automatic conversion of incoming strings, to ensure they have
...
### Fixes
  ...
  - When Django uses a persistent cache (Redis, memcached, etc.), ensure the registry is up-to-date with current sources
  at startup
...
### Improvements
  ...
  - Some documentation has been added, to cover more features of the library. See <https://django-modern-rpc.rtfd.io/>
...
### Improvements
  ...
  - Error reporting has been improved. Correct error codes and messages are returned on usual failures. See module
