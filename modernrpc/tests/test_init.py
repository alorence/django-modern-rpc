...
from modernrpc.apps import check_required_settings_defined, check_logging_configuration
...
def test_logging_check_1(settings):

    assert len(check_logging_configuration(apps.get_app_config("modernrpc"))) == 0

    settings.MODERNRPC_ENABLE_LOGGING = True

    result = check_logging_configuration(apps.get_app_config("modernrpc"))
    assert len(result) == 1
    assert result[0].id == 'modernrpc.E002'


def test_logging_check_2(settings):
    ...
    result = check_logging_configuration(apps.get_app_config("modernrpc"))
    assert len(result) == 1
    assert result[0].id == 'modernrpc.E003'
