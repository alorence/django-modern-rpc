...
from modernrpc.exceptions import (
    ...
    RPC_INVALID_REQUEST, RPC_PARSE_ERROR,
    RPC_CUSTOM_ERROR_BASE,
)


class TestCommonErrors:
    @pytest.mark.parametrize("method, params, exc_match, exc_code", [
        ("non_existing_method", [], r'Method not found: "non_existing_method"', RPC_METHOD_NOT_FOUND),
        ("add", [42], r'Invalid parameters: add\(\) missing 1 required positional argument', RPC_INVALID_PARAMS),
        ("add", [5, 9, 6], r'.+add\(\) takes 2 positional arguments but 3 were given', RPC_INVALID_PARAMS),
        ("divide", [50, 0], r'Internal error: division by zero', RPC_INTERNAL_ERROR),
        ("get_invalid_result", [], r'Unable to serialize result', RPC_INTERNAL_ERROR),
        ("raise_custom_exception", [], r'This is a test error', RPC_CUSTOM_ERROR_BASE + 5),
        # Data validity will be checked in JSON-RPC specific class:
        ("raise_custom_exception_with_data", [], r'This exception has additional data', RPC_CUSTOM_ERROR_BASE + 5),
    ])
...
class TestJsonRpcSpecificBehaviors:
    ...
    def test_jsonrpc_json_payload_invalid(self, live_server, endpoint_path, jsonrpc_content_type):
        ...
        response = self.low_level_jsonrpc_call(
            live_server.url + endpoint_path, payload=None, headers=headers, raw_payload=invalid_json_payload
        )
    ...
    def test_jsonrpc_invalid_payload_type(self, live_server, endpoint_path, jsonrpc_content_type):
    ...
    def test_jsonrpc_no_content_type(self, live_server, endpoint_path):
        ...
        assert "Unable to handle your request, the Content-Type header is mandatory" in exc_info.value.doc
