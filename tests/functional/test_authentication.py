...
class TestAuthSuperuser:
    ...
    @pytest.mark.parametrize("method_name, args, result", [
...
class TestAuthAnonymousUser:
    ...
    def test_with_anonymous_user(self, any_rpc_client, method_name, args):
        ...
        exc_match = r'Authentication failed when calling "{func}"'.format(func=method_name)
...
class TestAuthStandardUser:
    ...
    @pytest.mark.parametrize("method_name, args, must_raise, result", [
    ...
    def test_permissions_updates(self, any_rpc_client, john_doe, delete_user_perm, add_user_perm, change_user_perm):
