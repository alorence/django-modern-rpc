...
from modernrpc.exceptions import RPC_INTERNAL_ERROR, RPC_METHOD_NOT_FOUND, RPC_INVALID_REQUEST
...
def test_jsonrpc_batch_with_zero_division_error(jsonrpc_client):
    ...
    assert result == [
        {'id': 0, 'jsonrpc': '2.0', 'result': 15},
        {'id': 1, 'jsonrpc': '2.0', 'error': {
            'code': RPC_INTERNAL_ERROR, 'message': 'Internal error: division by zero'
        }},
        {'id': 2, 'jsonrpc': '2.0', 'result': 16},
    ]
...
def test_jsonrpc_batch_invalid_request(live_server, endpoint_path):
    ...
    result = requests.post(live_server.url + endpoint_path, data='[1, 2, 3]', headers=headers).json()
    ...
    assert result == [
        {'id': None, 'jsonrpc': '2.0', 'error': {'code': RPC_INVALID_REQUEST, 'message': expected_error_message}},
        {'id': None, 'jsonrpc': '2.0', 'error': {'code': RPC_INVALID_REQUEST, 'message': expected_error_message}},
        {'id': None, 'jsonrpc': '2.0', 'error': {'code': RPC_INVALID_REQUEST, 'message': expected_error_message}},
    ]
